...
const BackingTrack = () => {
	...
	const fetchBackingTrack = async () => {
		await apiClient
			...
			.then(() => {
				apiClient
					.get(getSocialVideoLinkEndPoint + "Backing_Track")
					.then((response) => {
	...
	const createModalData = (link, title, desc) => {
		...
		let yt_embed = (
			<>
				<YouTube videoId={video_id} opts={{ height: 640, width: 800 }} />
						...
						(getSearch &&
							...
							<div
								...
								<div className='w-118 h-60 overflow-hidden tablet:w-96 tablet:h-96 mobile:w-96 mobile:h-96'>
									<Image src={elems.thumbnail_url} width={5472} height={3648} layout='responsive' objectFit='cover' className='' />
								</div>
								{/* description */}
								<h2 className='py-6 large:px-16 mobile:px-32 large:pt-0 mobile:pt-0 text-center font-bold'>{elems.title}</h2>
							</div>
						) : (
							""
						)
